...
HCDotMatrixAComun::HCDotMatrixAComun(byte R0,byte R1,byte R2,byte R3,byte R4,byte R5,byte R6,byte R7,
						 byte C0,byte C1,byte C2,byte C3,byte C4,byte C5,byte C6,byte C7)
{
  ...
  for (_Cur_Row = 0; _Cur_Row < 8; _Cur_Row++)
  {
    ...
    digitalWrite(_PinOut_Row[_Cur_Row], HIGH);
  }
...
void HCDotMatrixAComun::UpdateMatrix(void)
{

  /* Cycles through each row in turn */
  for (_Cur_Col = 0; _Cur_Col < 8; _Cur_Col++)
  {
    /* Sets the appropriate state for the current row to LOW to enable
       the row */
    ...
    for (_Cur_Row = 0; _Cur_Row < 8; _Cur_Row++)
    {
      /* Sets the column state for the current LED to HIGH to turn the
         LED on */
...
void HCDotMatrixAComun::print(char TextString[], unsigned int xoffset)
{
